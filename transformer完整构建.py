"""transformer  手动搭建
主要包括四个模块:embedding \ encoder\ decoder \output   特征提取
"""
import torch
...
class Config(object):
    def __init__(self):
        ...
        self.d_model = 20  #embedding dim
        self.n_heads = 2   #2头注意力
...
class Embedding(nn.Module):
    """
    嵌入层
    在文本输入之前，我们需要进行padding统一长度 ,短补长截
    原始文本特征那个输入-->embedding+position embedding-->最终的目标embedding tensor
    """
    ...
    def forward(self,x):
        ...
        return x
# model_encod=Embedding(vocab_size=config.vocab_size)
# print(model_encod.forward([1,2,3,4,5,6]))

class Positional_Encoding(nn.Module):
    """
    生成额外的位置编码  根据计算公式得到
    Positional Embedding，我们需要参考论文给出的公式,
    在作者的实验中对比了Positional Embedding与单独采用一个Embedding训练模型对位置的感知两种方式，模型效果相差无几。
    """
...
model=Positional_Encoding(d_model=config.d_model)
print(model.forward(seq_len=config.vocab_size,embedding_dim=config.d_model))  #6*20 的位置编码


"""encoder部分,trans的核心内容
Encoder 中的 Muti_head_Attention 不需要Mask
为了避免模型信息泄露的问题，Decoder 中的 Muti_head_Attention 需要Mask
"""
...
"""decoder部分   与encoder区别的是多头注意力使用了MASK机制,凭借方式不同"""
...
#组装transformer   堆叠了6个我们上文中实现的Encoder 和 Decoder
